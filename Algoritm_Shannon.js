...
let token = Object.keys(vec).sort().reverse().sort((a, b) => {
    return vec[a] - vec[b];
}).reverse();
...
let treeData = treeBuild(sort, 0, token.length, "O", "A", 0)
design(treeData);
}

function treeBuild(sort, l, nr, pn, name, lvl) {
    let node = {};
    ...
    let poz;
    for (let i = l; i < nr; i++) {
        if (s + sort[keys[i]] < sum/2) {
            s = s + sort[keys[i]];
        } else {
            if (sum/2 - s < s + sort[keys[i]] - sum/2) {
                poz = i;
                break;
            }
            poz = i + 1;
    ...
    node["children"] =
        [treeBuild(sort, l, poz, "L", node["name"], lvl + 1), treeBuild(sort, poz, nr, "R", node["name"], lvl + 1)];

    return node;
}


//Codul preluat din laborator

function design(treeData) {
